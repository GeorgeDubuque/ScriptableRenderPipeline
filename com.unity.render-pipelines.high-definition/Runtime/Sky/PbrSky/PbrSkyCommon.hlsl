...
// V must be in the local sphere where (V.z = NdotV).
float MapCosineOfAzimuthAngle(float3 V)
{
    float phiV = atan2(abs(V.y), V.x); // Utilize the rotational symmetry

    // TODO: uniform mapping?
    // float x = phiV * INV_PI;
    // float y = 1 - 2 * x;
    // float s = FastSign(V.x);
...
float UnmapAzimuthAngle(float u)
{
